$(document).ready(function(){
    ...
    for(let i = 0;i < items.length ;i++){                                                  // first loop
        const ul = $('<ul/>');
        const li = $('<li/>');
        const a = $('<a>', { href:`#${items[i].name}`, text: '+', class:`${items[i].name}`});
        a.attr({
                'data-toggle': "collapse",
                'onclick': `collapse('${items[i].name}')`
        });
        const img = $('<img>', {src: image.src, width: image.width, height: image.height});
        const div = $('<div/>',{id:`${items[i].name}`, class:'collapse'});
        const text = $('<a>', { href:`#`, text: `${items[i].name}`, class:`demo${i + 1}`});
        ...
        for(let j = 0;j < items[i].children.length; j++){                                  // second loop
            const ul2 = $('<ul/>');
            const li2 = $('<li/>');
            const a2 = $('<a>', { href: `#${items[i].children[j].name}`, text: '+', class:`${items[i].children[j].name}`});
            a2.attr({
                    'data-toggle': "collapse",
                    'onclick': `collapse('${items[i].children[j].name}')`
            });
            const img2 = $('<img>', {src:image.src ,width: image.width, height: image.height});
            const div2 = $('<div/>',{id:`${items[i].children[j].name}`, class:'collapse'});
            ...
            for(let k = 0;k < items[i].children[j].children2.length;k++){                  //  third loop
                const ul3 = $('<ul/>');
                const li3 = $('<li/>');
                const a3 = $('<a>', { href: `#`, text: items[i].children[j].children2[k].name});
                a3.css({'color': '#646464','cursor': 'auto'});
                a3.attr({
                        'data-toggle': "collapse",
                        'onclick': `collapse('${items[i].children[j].children2[k].name}')`
                });
                const img3 = $('<img>', {src:image.src ,width: image.width, height: image.height});
                const div3 = $('<div/>',{id:`${items[i].children[j].children2[k].name}`, class:'collapse'});
                li3
                    .append(img3)
                    .append(a3)
                    .append(div3)
